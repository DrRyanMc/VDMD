...
def multigroup_k(I,hx,G,sigma_t,sigma_s,nusigma_f,chi,N,BCs,data_struct=None, phi = np.zeros(1), tolerance = 1.0e-8,maxits = 100, LOUD=False ):
    ...
    if (data_struct is not None):
        ...
        phi_fast = np.zeros(I)
        for i in range(I):
            phi_thermal[i] = np.sum(phi_old[i,data_struct.group_edges[0:G]<=(0.55e-6)])
            phi_fast[i] = np.sum(phi_old[i,data_struct.group_edges[1:(G+1)]>=(1)])
            phi_epithermal[i] = np.sum(phi_old[i,:]) - phi_thermal[i] - phi_fast[i]
            #np.sum(phi_old[i,(data_struct.group_edges[1:(G+1)]<=(1)) * (data_struct.group_edges[0:G]>=(0.05e-6))])
        return x, k, phi_old, phi_thermal, phi_epithermal,phi_fast
    return x, k, phi_old

def multigroup_alpha(I,hx,G,sigma_t,sigma_s,nusigma_f,chi,N,BCs,inv_speed,min_alpha,max_alpha,data_struct=None,phi = np.zeros(1), tolerance = 1.0e-8,maxits = 100, LOUD=False, atol=1e-5 ):
...
def multigroup_td(I,hx,G,sigma_t,sigma_s,nusigma_f,chi,inv_speed,N,BCs,psi0,qiso,numsteps,dt,data_struct=None, tolerance = 1.0e-8,maxits = 100, LOUD=False ):
    """Solve k eigenvalue problem
    Inputs:
        I:               number of zones 
        hx:              size of each zone
    ...
    for step in range(numsteps):
        ...
        if (LOUD>=1):
            ...
            if (data_struct is not None):
                ...
                phi_fast = np.zeros(I)
                for i in range(I):
                    phi_thermal[i] = np.sum(phi_out[i,data_struct.group_edges[0:G]<=(0.55e-6),step])
                    phi_fast[i] = np.sum(phi_out[i,data_struct.group_edges[1:(G+1)]>=(1),step])
                    phi_epithermal[i] = np.sum(phi_out[i,(data_struct.group_edges[1:(G+1)]<=(1)) * (data_struct.group_edges[0:G]>=(0.05e-6)),step])
...
def multigroup_td_var(I,hx,G,sigma_t,sigma_s,nusigma_f,chi,inv_speed,N,BCs,psi0,qiso,dt,data_struct=None, tolerance = 1.0e-8,maxits = 100, LOUD=False ):
    """Solve k eigenvalue problem
    Inputs:
        I:               number of zones 
        hx:              size of each zone
    ...
    numsteps = dt.size
    phi_out = np.zeros((I,G,numsteps))
    psi_out = np.zeros((I,N,G,numsteps+1))
    phi = np.zeros((I,G))
    psi = np.zeros((I,N,G))
    if (psi0.size == I*N*G):
        psi = psi0.copy()
    ...
    for step in range(numsteps):
        #compute fission source
        for angle in range(N):
            Q[:,angle,:] = q[:,angle,:] + psi[:,angle,:]*inv_speed/dt[step]
        
        t += dt[step]
        x,phi_out[:,:,step], psi = multigroup_ss_td(I,hx,G,Q,sigma_t+1/dt[step]*inv_speed,sigma_s,nusigma_f,chi,N,BCs, tolerance = tolerance*0.01,maxits = 100, LOUD=LOUD-1 )
        psi_out[:,:,:,step+1] = psi.copy()
        if (LOUD>=1):
            print("**********************\nStep", step,"t =",t,"\n**********************")
            #plt.plot(x,phi_out[:,:,step])
            #plt.show()
